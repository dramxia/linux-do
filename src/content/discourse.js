...
export function getNativePostElements() {
  return getAllPostElements()
    .filter((postEl) => !postEl.classList.contains('ldtk-paged-comment'));
}

export function getPostMeta(postEl) {
...
export async function fetchRawPost(topicId, postNumber) {
...
export async function fetchTopicJson(topicId) {
...
export async function fetchPostsByIds(topicId, postIds) {
  if (!topicId) throw new Error('缺少主题 ID');
...
export function getPostImages(postEl) {
  const images = {};
...
export function replaceUploadUrls(rawMd, imageMap) {
