...
- **正文 / 评论分栏**：将原生主题正文固定在左侧，原生评论流显示在右侧；窄屏会自动恢复单列布局。
...
开启后会默认收起 Linux.do 左侧边栏。分栏直接复用页面已经挂载的原生正文和评论节点，不复制或重新拼装楼层，因此点赞、回复、书签、更多菜单以及工具箱按钮仍使用原来的交互。正文内容与右侧评论流分别滚动，正文和主题操作区固定在左栏上下两端；视口宽度不适合双栏时会自动切换为单列。
...
导出范围由当前页面已渲染楼层决定，不会主动遍历服务器上的所有回复。分栏模式复用同一批原生楼层，因此会包含左侧正文和右侧当前已渲染的评论；尚未被 Discourse 懒加载出来的楼层仍不会导出。
...
- **导出楼层不全**：弹窗导出只处理当前已加载楼层；请先在右侧评论流中滚动到需要的楼层，让 Discourse 完成懒加载。
...
├── content/layout/           # 原生正文 / 评论分栏与恢复生命周期
