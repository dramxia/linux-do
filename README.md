...
1. 在 Linux.do 主题页打开扩展弹窗，开启“主题正文 / 评论双栏阅读”；页面会自动刷新。
2. 视口宽度至少为 `1280px` 时，首帖正文显示在左栏，回复按 10 条或 20 条分页显示在右栏，两栏可独立滚动。
...
| 选项 | 默认值 | 作用 |
| --- | --- | --- |
| 主题正文 / 评论双栏阅读 | 关闭 | 在桌面宽屏主题页启用正文和分页评论双栏；切换后刷新当前标签页 |
| 每页评论数 | 10 条 | 双栏右侧每页显示 10 条或 20 条评论 |
...
修改普通设置后会立即通知当前页面；修改双栏阅读总开关会刷新当前标签页。设置会写入 `chrome.storage.sync`，因此会随浏览器配置同步。
...
新增需要随页面 DOM 更新的功能时，实现一个接收 `DiscourseSettings` 的刷新函数，并在 `content/index.ts` 的 `enhancements` 中连同自有 DOM 选择器一起注册。新增设置项时同步更新设置接口、`SETTING_KEYS`、默认值和 popup 控件，入口会把同一份设置快照传给所有刷新器。
